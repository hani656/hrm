  ...
  <div class="container">
    <div class="app-container">
      ...
      <el-tree :expand-on-click-node="false" default-expand-all :data="depts" :props="defaultProps">
        <!-- 节点结构 -->
        <!-- v-slot="{node, data}" 只能作用于template-->
  ...
  methods: {
    ...
    operateDept(type, id) {
      ...
      if (type === 'add') {
        ...
        // 要在子组件获取数据
        // 父组件调用子组件的方法来获取数据
        // this.$nextTick( ()=>{} )等上面的异步执行完再执行此回调函数（默认先执行同步，再执行异步）
        this.$nextTick(() => {
